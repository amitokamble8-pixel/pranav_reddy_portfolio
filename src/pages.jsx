  ...
  <section className="hero">
    <div className="hero-inner">
      ...
      <div className="cta-row">
        <Link className="btn btn-primary" to="/work">Academics <Arrow /></Link>
        {PROFILE.cv && (
          <a className="btn btn-ghost" href={PROFILE.cv} download> Download CV<DownloadIcon /></a>
        )}
        {PROFILE.email && (
          <a className="btn btn-ghost" href={`mailto:${PROFILE.email}`}>Email<MailIcon size={16} /></a>
        )}
      </div>
    </div>
  </section>
...
const ConnectPanel = () => (
  <Reveal>
    <div className="connect">
      ...
      <p>
        For collaborations, academic opportunities, or technology-driven projects or 
        simply to discuss computer science, artificial intelligence, mathematics, or innovation—I'd love to connect.
...
export function Home() {
  ...
  return (
    <>
      ...
      <section className="section">
        <SectionHead title="Academic Enrichment" eyebrow="Courses · Programs" />
        <div className="grid-2">
          {EXPERIENCE.filter((e) => e.featured).slice(0, 2).map((e, i) => (
            <Reveal key={e.slug} delay={i * 90}><ExperienceCard e={e} /></Reveal>
          ))}
        </div>
        <MoreLink to="/work">More about Academic Enrichment</MoreLink>
      </section>

      <section className="section">
        <SectionHead title="Featured Projects" eyebrow="Designed · Programmed" />
        <div className="grid-2">
          {PROJECTS.filter((p) => p.featured).map((p, i) => (
            <Reveal key={p.name} delay={i * 90}><ProjectCard p={p} /></Reveal>
          ))}
        </div>
        <MoreLink to="/projects">All Featured Projects</MoreLink>
      </section>
      ...
      <section className="section">
        <SectionHead title="Areas of Interest" eyebrow="What I'm Curious About" />
        <div className="row-list">
          {ARTICLES.map((a, i) => (
            <Reveal key={a.title} delay={i * 60}><ArticleRow a={a} /></Reveal>
          ))}
        </div>
        <MoreLink to="/publications">More about Areas of Interest</MoreLink>
      </section>

      <section className="section">
        <SectionHead title="Community & Leadership" eyebrow="Teamwork · Impact" />
        <Reveal>
        ...
        <MoreLink to="/volunteering">More about Community & Leadership</MoreLink>
      </section>
...
export function About() {
  ...
  return (
    <section className="section">
      <PageHead title="About" eyebrow="Gateway The Complete School (IGCSE)" />
      ...
      <SubHead title="Education" eyebrow="Chennai, Tamil Nadu, India" />
      <div className="stack">
        {EDUCATION.map((e, i) => (
...
export function Work() {
  usePageMeta(`Academic Enrichment | ${SITE}`, "Research assistantships, work experience, and teaching roles.");
  return (
    <section className="section">
      <PageHead title="Academic Enrichment" eyebrow="Courses · Programs" />
      <div className="grid-2">
        {EXPERIENCE.map((e, i) => (
...
export function Projects() {
  usePageMeta(`FeaturedProjects | ${SITE}`, "Research and software projects, from wearable diagnostics to mobile applications.");
  return (
    <section className="section">
      <PageHead title="Featured Projects" eyebrow="Designed · Programmed" />
      <div className="grid-2">
        {PROJECTS.map((p, i) => (
...
export function Publications() {
  usePageMeta(`Areas of Interest | ${SITE}`, "Areas of research and technical interest.");
  return (
    <section className="section">
      <PageHead title="Areas of Interest" eyebrow="What I'm Curious About" />
      <div className="row-list">
        {ARTICLES.map((a, i) => (
          <Reveal key={a.title} delay={i * 60}><ArticleRow a={a} /></Reveal>
        ))}
...
export function Volunteering() {
  usePageMeta(`Community & Leadership | ${SITE}`, "Founding a machine learning club, teaching summer band, and community volunteering.");
  return (
    <section className="section">
      <PageHead title="Community & Leadership" eyebrow="Teamwork · Impact" />
      <Reveal>
        <div className="vol-panel">
          <div className="vol-stats">
            {VOLUNTEER.stats.map((s) => (
